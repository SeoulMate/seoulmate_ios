...
#import "KoreanListViewController.h"
//#import "FeedCell.h"
#import "KoreanCell.h"
...
@interface KoreanListViewController ()
@property (nonatomic, assign) BOOL shouldReloadOnAppear;
@property (nonatomic, strong) NSMutableSet *reusableSectionHeaderViews;
@property (nonatomic, strong) NSMutableDictionary *outstandingSectionHeaderQueries;
@end

@implementation KoreanListViewController
...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if (self) {
        ...
        self.parseClassName = kPAPKoreanClassKey;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [tracker set:kGAIScreenName value:kAnalyticsKoreanVC];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
    static NSString *CellIdentifier = @"KoreanCell";
    ...
    if (indexPath.row % 2 == 0) {
        ...
        // Photo
        KoreanCell *cell = (KoreanCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        
        if (cell == nil) {
            cell = [[KoreanCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        ...
        [cell.postTags setText:[NSString stringWithFormat:@"# %@",[object objectForKey:@"tags"]]];
...
- (void)photoHeaderView:(PAPPhotoHeaderView *)photoHeaderView didTapCommentOnPhotoButton:(UIButton *)button  photo:(PFObject *)photo {
    KoreanDetailsViewController *koreanDetailsVC = [[KoreanDetailsViewController alloc] initWithKorean:photo korean:photo];
    [self.navigationController pushViewController:koreanDetailsVC animated:YES];
}
...
- (UITableViewCell *)detailPhotoCellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *CellIdentifier = @"DetailKoreanCell";
    ...
    if (attributesForKorean) {
        ...
        @synchronized(self) {
            ...
            if (!outstandingSectionHeaderQueryStatus) {
                PFQuery *query = [PAPUtility queryForActivitiesOnKorean:object cachePolicy:kPFCachePolicyNetworkOnly];
                [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
                    @synchronized(self) {
                        ...
                        [[PAPCache sharedCache] setAttributesForKorean:object likers:likers commenters:commenters likedByCurrentUser:isLikedByCurrentUser];
...
- (void)selectBackground:(int)cat boardCell:(KoreanCell *)boardCell {
