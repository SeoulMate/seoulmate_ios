...
@interface BoardDetailsViewController ()
@property (nonatomic, strong) UITextField *commentTextField;
@property (nonatomic, strong) BoardDetailsHeaderView *headerView;
@property (nonatomic, assign) BOOL likersQueryInProgress;
@end

static const CGFloat kPAPCellInsetWidth = 0.0f;

@implementation BoardDetailsViewController

@synthesize commentTextField;
@synthesize photo,board, headerView;
...
- (id)initWithBoard:(PFObject *)aPhoto board:(PFObject *)aBoard {
    ...
    if (self) {
        ...
        self.parseClassName = kPAPActivityBoardClassKey;
...
- (void)viewDidLoad {
    ...
    if ([imageFile isEqual:[NSNull null]]) {
        ...
        self.headerView = [[BoardDetailsHeaderView alloc] initWithFrame:[BoardDetailsHeaderView rectForViewWithoutImage] photo:self.photo];
    } else {
        if([imageFile url]) {
                self.headerView = [[BoardDetailsHeaderView alloc] initWithFrame:[BoardDetailsHeaderView rectForView] photo:self.photo];
        } else {
            self.headerView = [[BoardDetailsHeaderView alloc] initWithFrame:[BoardDetailsHeaderView rectForViewWithoutImage] photo:self.photo];
        }
    }
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [tracker set:kGAIScreenName value:kAnalyticsBoardDetailsVC];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    BOOL hasCachedLikers = [[PAPCache sharedCache] attributesForBoard:self.photo] != nil;
    if (!hasCachedLikers) {
        [self loadLikers];
    }
}
...
- (PFQuery *)queryForTable {
    ...
    [query whereKey:kPAPActivityBoardKey equalTo:self.board];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (trimmedComment.length != 0 && [self.board objectForKey:kPAPPhotoUserKey]) {
        PFObject *comment = [PFObject objectWithClassName:kPAPActivityBoardClassKey];
        [comment setObject:trimmedComment forKey:kPAPActivityContentKey]; // Set comment text
        [comment setObject:[self.board objectForKey:kPAPPhotoUserKey] forKey:kPAPActivityToUserKey]; // Set toUser
        [comment setObject:[PFUser currentUser] forKey:kPAPActivityFromUserKey]; // Set fromUser
        [comment setObject:kPAPActivityTypeComment forKey:kPAPActivityTypeKey];
        [comment setObject:self.board forKey:kPAPActivityBoardKey];
        
        PFACL *ACL = [PFACL ACLWithUser:[PFUser currentUser]];
        [ACL setPublicReadAccess:YES];
        [ACL setWriteAccess:YES forUser:[self.board objectForKey:kPAPPhotoUserKey]];
        comment.ACL = ACL;
        
        [[PAPCache sharedCache] incrementCommentCountForBoard:self.board];
...
//        NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:5.0f target:self selector:@selector(handleCommentTimeout:) userInfo:@{@"comment": comment} repeats:NO];
        ...
        [comment saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
            if (error && error.code == kPFErrorObjectNotFound) {
                [[PAPCache sharedCache] decrementCommentCountForBoard:self.photo];
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Could not post comment", nil) message:NSLocalizedString(@"This board post is no longer available", nil) delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
                [alert show];
                [self.navigationController popViewControllerAnimated:YES];
            }
            
            [[NSNotificationCenter defaultCenter] postNotificationName:PAPPhotoDetailsViewControllerUserCommentedOnPhotoNotification object:self.board userInfo:@{@"comments": @(self.objects.count + 1)}];
...
#pragma mark - BoardDetailsHeaderViewDelegate
...
- (void)showShareSheet {
    [[self.photo objectForKey:kPAPPhotoPictureKey] getDataInBackgroundWithBlock:^(NSData *data, NSError *error) {
        if (!error) {
            ...
            // Prefill caption if this is the original poster of the post, and then only if they added a caption initially.
...
//            }
            ...
            [activityItems addObject:[NSURL URLWithString:[NSString stringWithFormat:@"https://seoulmateapp.co/collections/board/%@", self.photo.objectId]]];
...
- (BOOL)currentUserOwnsPhoto {
    return [[[self.board objectForKey:kPAPPhotoUserKey] objectId] isEqualToString:[[PFUser currentUser] objectId]];
}

- (void)shouldDeletePhoto {
    // Delete all activites related to this photo
    PFQuery *query = [PFQuery queryWithClassName:kPAPActivityBoardClassKey];
    [query whereKey:kPAPActivityBoardKey equalTo:self.photo];
...
- (void)sendOffensiveReport {
    ...
    PFObject *report = [PFObject objectWithClassName:kReportBoardClassKey];
    ...
    [report setObject:self.board forKey:kPAPActivityBoardKey];
