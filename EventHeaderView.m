...
- (id)initWithFrame:(CGRect)frame buttons:(EventHeaderButtons)otherButtons {
    ...
    if (self) {
        ...
        if (self.buttons & EventHeaderButtonsComment) {
            // comments button
...
//            [containerView addSubview:self.commentButton];
        ...
        if (self.buttons & EventHeaderButtonsLike) {
            // like button
...
- (void)setPhoto:(PFObject *)aPhoto {
    ...
    // user's avatar
    PFUser *user = [self.photo objectForKey:kPAPPostUserKey];
...
- (void)didTapUserButtonAction:(UIButton *)sender {
    if (delegate && [delegate respondsToSelector:@selector(eventHeaderView:didTapUserButton:user:)]) {
        [delegate eventHeaderView:self didTapUserButton:sender user:[self.photo objectForKey:kPAPPostUserKey]];
    }
}

- (void)didTapLikePhotoButtonAction:(UIButton *)button {
    if (delegate && [delegate respondsToSelector:@selector(eventHeaderView:didTapLikePhotoButton:photo:)]) {
        [delegate eventHeaderView:self didTapLikePhotoButton:button photo:self.photo];
    }
}

- (void)didTapCommentOnPhotoButtonAction:(UIButton *)sender {
    if (delegate && [delegate respondsToSelector:@selector(eventHeaderView:didTapCommentOnPhotoButton:photo:)]) {
        [delegate eventHeaderView:self didTapCommentOnPhotoButton:sender photo:self.photo];
    }
}
