...
#define mainImageHeight 310.0f

#define likeBarX baseHorizontalOffset
#define likeBarY nameHeaderHeight+mainImageHeight+10.0f
#define likeBarWithoutImageY nameHeaderHeight + titleHeight - 10.0f
...
#define viewTotalHeight likeBarY+likeBarHeight+titleHeight+contentHeight
...
@interface KoreanDetailsHeaderView ()
...
@property (nonatomic,strong) UILabel *koreanTitle;
@property (nonatomic,strong) UILabel *koreanContent;
...
@implementation KoreanDetailsHeaderView
...
@synthesize koreanTitle;
@synthesize koreanContent;
...
- (void)reloadLikeBar {
    self.likeUsers = [[PAPCache sharedCache] likersForKorean:self.photo];
//    NSLog(@"likeUsers : %@",self.likeUsers);
    [self setLikeButtonState:[[PAPCache sharedCache] isKoreanLikedByCurrentUser:self.photo]];
...
- (void)createView {
    ...
    self.koreanTitle = [[UILabel alloc] initWithFrame:CGRectMake(titleHorizontalOffset, titleMarginY, titleWidth, titleHeight)];
    
    // Configure Main Label
    [self.koreanTitle setFont:[UIFont boldSystemFontOfSize:18.0]];
    [self.koreanTitle setTextAlignment:NSTextAlignmentCenter];
    self.koreanTitle.numberOfLines = 4;
    [self.koreanTitle setTextColor:[UIColor blackColor]];
    [self.koreanTitle setAutoresizingMask:(UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight)];
    //  self.boardTitle.backgroundColor = [UIColor colorWithRed:25.0f/255.0f green:188.0f/255.0f blue:241.0f/255.0f alpha:1.0f];
    [self.koreanTitle setText:[self.photo objectForKey:@"title"]];
    self.koreanContent = [[UILabel alloc] initWithFrame:CGRectMake(titleHorizontalOffset, contentY, contentWidth, 100.0f)];
    [self.koreanContent setFont:[UIFont systemFontOfSize:12.0f]];
    [self.koreanContent setTextColor:[UIColor blackColor]];
    self.koreanContent.lineBreakMode = NSLineBreakByWordWrapping;
    [self.koreanContent setBackgroundColor:[UIColor clearColor]];
    ...
    [self.koreanContent setText:content];
    
    self.koreanContent.preferredMaxLayoutWidth = self.frame.size.width; // assumes the parent view has its frame already set.

    UILabel *gettingSizeLabel = [[UILabel alloc] init];
    gettingSizeLabel.font = koreanContent.font; // Your Font-style whatever you want to use.
    gettingSizeLabel.text = self.koreanContent.text;
    gettingSizeLabel.numberOfLines = 0;
    CGSize maximumLabelSize = CGSizeMake(310, 9999);
    CGSize expectedLabelSize = [gettingSizeLabel sizeThatFits:maximumLabelSize];
    
    CGRect newFrame = self.koreanContent.frame;
    newFrame.size.height = expectedLabelSize.height;
    self.koreanContent.frame = newFrame;
    self.koreanContent.numberOfLines = 0;
    [self.koreanContent sizeToFit];
    
    NSLog(@"title height : %02f contentHeight: %02f length: %lu",self.koreanTitle.bounds.size.height,expectedLabelSize.height, (unsigned long)self.koreanContent.text.length);
    
    [self addSubview:self.koreanTitle];
    [self addSubview:self.koreanContent];
...
- (void)didTapLikePhotoButtonAction:(UIButton *)button {
    ...
    if (liked) {
        [[PAPCache sharedCache] incrementLikerCountForKorean:self.photo];
        [newLikeUsersSet addObject:[PFUser currentUser]];
    } else {
        [[PAPCache sharedCache] decrementLikerCountForKorean:self.photo];
    }
    
    [[PAPCache sharedCache] setKoreanIsLikedByCurrentUser:self.photo liked:liked];
    
    [self setLikeUsers:[newLikeUsersSet allObjects]];
    
    if (liked) {
        [PAPUtility likeKoreanInBackground:self.photo block:^(BOOL succeeded, NSError *error) {
            if (!succeeded) {
        ...
        [PAPUtility unlikeKoreanInBackground:self.photo block:^(BOOL succeeded, NSError *error) {
            if (!succeeded) {
                [button addTarget:self action:@selector(didTapLikePhotoButtonAction:) forControlEvents:UIControlEventTouchUpInside];
                [self setLikeUsers:originalLikeUsersArray];
...
- (void)didTapLikerButtonAction:(UIButton *)button {
    ...
    if(delegate && [delegate respondsToSelector:@selector(koreanDetailsHeaderView:didTapUserButton:user:)]) {
        [delegate koreanDetailsHeaderView:self didTapUserButton:button user:user];
    }
...
- (void)didTapUserNameButtonAction:(UIButton *)button {
    if (delegate && [delegate respondsToSelector:@selector(koreanDetailsHeaderView:didTapUserButton:user:)]) {
        [delegate koreanDetailsHeaderView:self didTapUserButton:button user:self.writer];
    }    
}

- (void)didTapReadMoreButtonAction:(UIButton *)button {
    
    if (delegate && [delegate respondsToSelector:@selector(koreanDetailsHeaderView:didTapReadMoreButton:)]) {
        [delegate koreanDetailsHeaderView:self didTapReadMoreButton:button];
    }
}
