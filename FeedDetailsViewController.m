...
@property (nonatomic, strong) FeedDetailsHeaderView *headerView;
...
@synthesize photo,feed, headerView;
...
- (void)viewDidLoad {
    ...
    if ([imageFile isEqual:[NSNull null]]) {
        self.headerView = [[FeedDetailsHeaderView alloc] initWithFrame:[FeedDetailsHeaderView rectForViewWithoutImage] photo:self.photo];
    } else {
         if([imageFile url]) {
             self.headerView = [[FeedDetailsHeaderView alloc] initWithFrame:[FeedDetailsHeaderView rectForView] photo:self.photo];
         } else {
             self.headerView = [[FeedDetailsHeaderView alloc] initWithFrame:[FeedDetailsHeaderView rectForViewWithoutImage] photo:self.photo];
         }
    }
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [tracker set:kGAIScreenName value:kAnalyticsFeedDetailsVC];
...
- (PFQuery *)queryForTable {
    ...
    [query whereKey:kPAPActivityFeedKey equalTo:self.feed];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (trimmedComment.length != 0 && [self.feed objectForKey:kPAPPhotoUserKey]) {
        PFObject *comment = [PFObject objectWithClassName:kPAPActivityClassKey];
        [comment setObject:trimmedComment forKey:kPAPActivityContentKey]; // Set comment text
        [comment setObject:[self.feed objectForKey:kPAPPhotoUserKey] forKey:kPAPActivityToUserKey]; // Set toUser
        [comment setObject:[PFUser currentUser] forKey:kPAPActivityFromUserKey]; // Set fromUser
        [comment setObject:kPAPActivityTypeComment forKey:kPAPActivityTypeKey];
        [comment setObject:self.feed forKey:kPAPActivityFeedKey];
        
        PFACL *ACL = [PFACL ACLWithUser:[PFUser currentUser]];
        [ACL setPublicReadAccess:YES];
        [ACL setWriteAccess:YES forUser:[self.feed objectForKey:kPAPPhotoUserKey]];
        comment.ACL = ACL;
        
        [[PAPCache sharedCache] incrementCommentCountForFeed:self.feed];
        ...
        [comment saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:PAPPhotoDetailsViewControllerUserCommentedOnPhotoNotification object:self.feed userInfo:@{@"comments": @(self.objects.count + 1)}];
...
#pragma mark - FeedDetailsHeaderViewDelegate

-(void)photoDetailsHeaderView:(FeedDetailsHeaderView *)headerView didTapUserButton:(UIButton *)button user:(PFUser *)user {
...
- (void)showShareSheet {
    [[self.photo objectForKey:kPAPPhotoPictureKey] getDataInBackgroundWithBlock:^(NSData *data, NSError *error) {
        if (!error) {
            ...
            [activityItems addObject:[self.photo objectForKey:@"content"]];
...
//            }
            ...
            [activityItems addObject:[NSURL URLWithString:[NSString stringWithFormat:@"https://seoulmateapp.co/feed/%@", self.photo.objectId]]];
...
- (void)loadLikers {
    ...
    PFQuery *query = [PAPUtility queryForActivitiesOnFeed:feed cachePolicy:kPFCachePolicyNetworkOnly];
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        ...
        [[PAPCache sharedCache] setAttributesForFeed:feed likers:likers commenters:commenters likedByCurrentUser:isLikedByCurrentUser];
        [self.headerView reloadLikeBar];
    }];
}

- (BOOL)currentUserOwnsPhoto {
    return [[[self.feed objectForKey:kPAPPhotoUserKey] objectId] isEqualToString:[[PFUser currentUser] objectId]];
}
...
- (void)sendOffensiveReport {
    ...
    PFObject *report = [PFObject objectWithClassName:kReportFeedClassKey];
    ...
    [report setObject:self.feed forKey:kPAPActivityFeedKey];
