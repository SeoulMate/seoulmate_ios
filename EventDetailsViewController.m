...
@property (nonatomic, strong) EventDetailsHeaderView *headerView;
...
- (void)viewDidLoad {
    ...
    if ([imageFile isEqual:[NSNull null]]) {
        ...
        self.headerView = [[EventDetailsHeaderView alloc] initWithFrame:[EventDetailsHeaderView rectForViewWithoutImage] photo:self.photo];
    } else {
        if([imageFile url]) {
            self.headerView = [[EventDetailsHeaderView alloc] initWithFrame:[EventDetailsHeaderView rectForView] photo:self.photo];
        } else {
            self.headerView = [[EventDetailsHeaderView alloc] initWithFrame:[EventDetailsHeaderView rectForViewWithoutImage] photo:self.photo];
        }
    }
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [tracker set:kGAIScreenName value:kAnalyticsEventsDetailsVC];
...
#pragma mark - EventDetailsHeaderViewDelegate
...
- (void)showShareSheet {
    [[self.photo objectForKey:kPAPPhotoPictureKey] getDataInBackgroundWithBlock:^(NSData *data, NSError *error) {
        if (!error) {
            ...
            [activityItems addObject:[NSURL URLWithString:[NSString stringWithFormat:@"https://seoulmateapp.co/event/%@", self.photo.objectId]]];
...
- (BOOL)currentUserOwnsPhoto {
    return [[[self.board objectForKey:@"writer"] objectId] isEqualToString:[[PFUser currentUser] objectId]];
}
...
- (void)sendOffensiveReport {
    ...
    PFObject *report = [PFObject objectWithClassName:kReportEventClassKey];
    ...
    [report setObject:self.board forKey:@"event"];
